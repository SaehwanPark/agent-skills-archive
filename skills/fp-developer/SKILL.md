...
For agent work, tests act as executable specs. Do not make broad refactors without first understanding or adding tests.
...
* `pymonad` or lightweight local `Result` / `Option` helpers
* `pytest` for tests

Add any missing dependencies via configured dependency manager (`uv` preferred).
...
Use structured results for recoverable failure.
...
5. Refactor toward small pure functions.
6. Keep adapters thin.
7. Run type checks and tests.
8. Report remaining violations clearly.
